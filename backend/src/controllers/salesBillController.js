...
function buildPdfBuffer(bill, customer, company, items) {
  return new Promise((resolve, reject) => {
    try {
      ...
      doc.font('Helvetica-Bold').text(`INVOICE: ${bill.bill_number}`, { underline: false });
      ...
      const [billInsert] = await conn.execute(
        `INSERT INTO sales_bills 
          (bill_number, bill_date, customer_id, tax_id, company_state_snapshot, customer_state_snapshot, is_interstate,
           subtotal, cgst_amount, sgst_amount, igst_amount, grand_total, paid_amount, balance_amount, status, notes)
         VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 0.00, ?, 'UNPAID', ?)`,
        [
          billNumber,
          bill_date,
          ...
          taxResult.grandTotal, // Initial balance_amount = grandTotal
          notes || null
        ]
      );
