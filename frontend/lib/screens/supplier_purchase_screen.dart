        ...
        children: [
          ...
          AppDataTable(
            ...
            columns: [
              ...
              AppTableColumn(title: 'Grand Total', builder: (p) => Text(Formatters.formatCurrency(double.tryParse(p['grand_total']?.toString() ?? '0')), style: const TextStyle(fontWeight: FontWeight.bold))),
              AppTableColumn(title: 'Paid', builder: (p) => Text(Formatters.formatCurrency(double.tryParse(p['paid_amount']?.toString() ?? '0')), style: const TextStyle(color: AppColors.success))),
              AppTableColumn(title: 'Balance', builder: (p) => Text(Formatters.formatCurrency(double.tryParse(p['balance_amount']?.toString() ?? '0')), style: TextStyle(color: (double.tryParse(p['balance_amount']?.toString()??'0')??0) > 0 ? AppColors.error : AppColors.success))),
            ],
          ),
        ],
      ),
